...
async def generate_post(profile: dict, topic: str,
    ...
    """Тема или референс -> {hooks, body}."""
    nl = chr(10)
    user = f"Тема поста: {topic}"
    if goal:
        user += nl + nl + "ЦЕЛЬ ЭТОГО ПОСТА: " + goal
    if reference:
        user += nl + nl + "Пост-референс (укради механику, не текст):" + nl + reference
    if recent:
        sep = nl + nl + "---" + nl + nl
        user += (nl + nl + "МОИ ПОСЛЕДНИЕ ПОСТЫ. Запрещено повторять их первые "
                 "строки, структуру, формулировки и заходы. Нужен ДРУГОЙ тип "
                 "хука и другая подача:" + nl + nl + sep.join(recent))
